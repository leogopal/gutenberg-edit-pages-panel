	...
	return (
		...
		>
			<Panel className="gutenberg-edit-pages-panel" initialOpen>
				<PanelBody
					title={ __( 'Search Pages', 'hostinger-easy-onboarding' ) }
					initialOpen
				>
					<SearchControl
...
const addPagesBtn = () => {
	try {
		const editPagesbutton = document.querySelector(
			'button[aria-controls="gutenberg-edit-pages:gutenberg_edit_pages"]'
		);
		const icon = `
		...
		if ( editPagesbutton ) {
			editPagesbutton.innerHTML =
				icon + ' ' + __( 'Edit Pages', 'hostinger-easy-onboarding' );
